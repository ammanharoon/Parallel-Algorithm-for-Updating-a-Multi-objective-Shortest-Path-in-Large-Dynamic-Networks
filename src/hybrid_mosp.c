...
void SOSP_Update_Hybrid(Graph* graph, SOSPTree* tree, Edge* inserted_edges, int num_inserted, int rank, int size, Partition* partition);
void computeInitialSOSP_Hybrid(Graph* graph, SOSPTree* tree, int source, int rank, int size, Partition* partition);
...
void MOSP_Update_2obj_Hybrid(Graph* graph1, Graph* graph2, SOSPTree* tree1, SOSPTree* tree2, 
...
// Graph partitioning using METIS with OpenMP for pre-processing
Partition* partitionGraphMETIS(Graph* graph, int rank, int size) {
    ...
    if (rank == 0) {
        // Convert graph to CSR format for METIS with OpenMP
        xadj = (idx_t*)malloc((n + 1) * sizeof(idx_t));
        
        // Count edges in parallel
        int* edge_counts = (int*)calloc(n, sizeof(int));
        ...
        xadj[0] = 0;
        for (int i = 0; i < n; i++) {
            xadj[i + 1] = xadj[i] + edge_counts[i];
        }
        
        int total_edges = xadj[n];
        adjncy = (idx_t*)malloc(total_edges * sizeof(idx_t));
        
        // Fill adjacency array - use OpenMP with atomic operations
        int* offsets = (int*)malloc(n * sizeof(int));
        memcpy(offsets, xadj, n * sizeof(int));
    ...
    // Set default values in parallel
    #pragma omp parallel for
    for (int i = 0; i < n; i++) {
    ...
    // Identify boundary vertices in parallel
    partition->boundary_vertices = (int*)malloc(local_count * sizeof(int));
    int* is_boundary = (int*)calloc(local_count, sizeof(int));
    ...
    // Compact the boundary vertices array
    partition->num_boundary = 0;
    for (int i = 0; i < local_count; i++) {
        if (is_boundary[i]) {
            partition->boundary_vertices[partition->num_boundary++] = i;
        }
    }
...
// Hybrid MPI+OpenMP SOSP_Update algorithm
void SOSP_Update_Hybrid(Graph* graph, SOSPTree* tree, Edge* inserted_edges, int num_inserted, 
                        int rank, int size, Partition* partition) {
    int n = graph->num_vertices;
    
    if (rank == 0) {
        printf("SOSP_Update: Hybrid MPI+OpenMP implementation with %d processes\n", size);
    }
    
    // Reset marked array in parallel
    #pragma omp parallel for
    for (int i = 0; i < n; i++) {
        tree->marked[i] = false;
    }
    
    // Step 0: Add edges to graph in parallel with synchronization
    #pragma omp parallel
    {
        #pragma omp for
    ...
    // Step 1: Process inserted edges in parallel with thread-local buffers
    #pragma omp parallel
    {
        // Thread-local storage
        bool* local_in_affected = (bool*)calloc(n, sizeof(bool));
        int* local_affected = (int*)malloc(n * sizeof(int));
        int local_size = 0;
        ...
        for (int i = 0; i < num_inserted; i++) {
            ...
            // Check if the distance to v can be improved (need atomic update)
            if (tree->distance[u] != INT_MAX) {
                bool improved = false;
                
                #pragma omp critical
                {
                    if (tree->distance[v] == INT_MAX || tree->distance[v] > tree->distance[u] + weight) {
                ...
                if (improved && !local_in_affected[v]) {
                    local_in_affected[v] = true;
                    local_affected[local_size++] = v;
                }
            }
        }
        
        // Merge thread-local buffers into global buffer
        #pragma omp critical
        {
            for (int i = 0; i < local_size; i++) {
                int v = local_affected[i];
                if (!in_affected[v]) {
                    in_affected[v] = true;
    ...
    // Step 2: Propagate updates with hybrid parallelism
    ...
    while (global_continue) {
        ...
        // Process current frontier with OpenMP
        #pragma omp parallel
        {
            // Thread-local buffers
            int* local_next = (int*)malloc(n * sizeof(int));
            int local_next_size = 0;
            bool* local_visited = (bool*)calloc(n, sizeof(bool));
            
            // Copy global visited array for thread-local use
            #pragma omp for
            for (int i = 0; i < n; i++) {
                local_visited[i] = visited[i];
            }
            
            // Process vertices in the current frontier in parallel
            #pragma omp for nowait
            for (int i = 0; i < affected_size; i++) {
                int u = affected[i];
                ...
                while (current) {
                    ...
                    // Try to relax edge (u,v) with atomic updates
                    if (tree->distance[u] != INT_MAX) {
                        bool improved = false;
                        ...
                        if (improved && !local_visited[v]) {
                            local_visited[v] = true;
                            local_next[local_next_size++] = v;
                        }
                    }
            ...
            // Merge thread-local results with global buffer
            #pragma omp critical
            {
        ...
        // Synchronize distances, parents, and marked arrays across all MPI processes
        ...
        // Update local arrays with global information in parallel
        #pragma omp parallel for
        for (int i = 0; i < n; i++) {
        ...
        if (rank == 0) {
            // Reset visited status for the new frontier in parallel
            #pragma omp parallel for
            for (int i = 0; i < n; i++) {
                visited[i] = false;
            }
...
// Hybrid MPI+OpenMP Bellman-Ford implementation
void computeInitialSOSP_Hybrid(Graph* graph, SOSPTree* tree, int source, 
                              int rank, int size, Partition* partition) {
    int n = graph->num_vertices;
    ...
    for (int iter = 0; iter < n - 1 && global_any_update; iter++) {
        ...
        // Each MPI process handles its partition with OpenMP parallelism
        #pragma omp parallel
        {
            bool thread_update = false;
            ...
            for (int idx = partition->start_vertex; idx <= partition->end_vertex; idx++) {
                ...
                while (current) {
                    ...
                    // Atomic update to tree->distance[v]
                    if (tree->distance[u] != INT_MAX && 
        ...
        // Synchronize distances and parents across MPI processes
        MPI_Allreduce(tree->distance, all_distances, n, MPI_INT, MPI_MIN, MPI_COMM_WORLD);
        MPI_Allreduce(tree->parent, all_parents, n, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
        
        // Update local copies in parallel
        #pragma omp parallel for
        for (int i = 0; i < n; i++) {
            tree->distance[i] = all_distances[i];
            tree->parent[i] = all_parents[i];
...
// Sequential computeInitialSOSP (for edge generation) - unchanged
void computeInitialSOSP(Graph* graph, SOSPTree* tree, int source) {
    int n = graph->num_vertices;
...
// Function to read graph from file with OpenMP parallelism
Graph* readGraph(const char* filename) {
    ...
    if (!edge_chunk) {
        fprintf(stderr, "Memory allocation failed for edge chunk\n");
        freeGraph(graph);
        fclose(file);
        return NULL;
    }
    ...
    Edge* createSmartRandomEdges(Graph* graph, int source, int target, int num_edges) {
        // Keep using sequential SOSP as requested
        int n = graph->num_vertices;
        ...
        // Find vertices reachable from source (can parallelize this part)
        int* reachable = (int*)malloc(n * sizeof(int));
        int reachable_count = 0;
        
        // Count reachable vertices in parallel
        int* local_counts = (int*)calloc(omp_get_max_threads(), sizeof(int));
        int** local_reachable = (int**)malloc(omp_get_max_threads() * sizeof(int*));
        
        #pragma omp parallel
        {
            int tid = omp_get_thread_num();
            local_reachable[tid] = (int*)malloc(n * sizeof(int));
            int local_count = 0;
            
            #pragma omp for nowait
            for (int i = 0; i < n; i++) {
                if (temp_tree->distance[i] != INT_MAX) {
                    local_reachable[tid][local_count++] = i;
                }
            }
        ...
        for (int i = 0; i < omp_get_max_threads(); i++) {
            for (int j = 0; j < local_counts[i]; j++) {
                reachable[reachable_count++] = local_reachable[i][j];
            }
            free(local_reachable[i]);
        }
        free(local_reachable);
        free(local_counts);
        
        // Check if target is reachable (sequential is fine for this small check)
        ...
        if (!target_reachable && reachable_count > 0) {
            ...
            // Build reversed graph in parallel
            #pragma omp parallel for
            for (int u = 0; u < n; u++) {
                AdjListNode* current = graph->array[u].head;
                while (current) {
            ...
            // Find vertices that can reach target in parallel
            int* can_reach_target = (int*)malloc(n * sizeof(int));
            int can_reach_count = 0;
            
            local_counts = (int*)calloc(omp_get_max_threads(), sizeof(int));
            local_reachable = (int**)malloc(omp_get_max_threads() * sizeof(int*));
            ...
            for (int i = 0; i < omp_get_max_threads(); i++) {
                for (int j = 0; j < local_counts[i]; j++) {
                    can_reach_target[can_reach_count++] = local_reachable[i][j];
                }
                free(local_reachable[i]);
            }
            free(local_reachable);
        ...
        // Fill the rest with random edges in parallel
        #pragma omp parallel for
        for (int i = connected_edges; i < num_edges; i++) {
            ...
            #pragma omp critical
            {
                if (reachable_count > 0) {
                    ...
                    if (rand() % 2 == 0) {
                        rnd_u = reachable[rand() % reachable_count];
                    } else {
                        rnd_u = rand() % n;
                    }
                } else {
    ...
    // Hybrid MPI+OpenMP MOSP_Update for 2 objectives
    void MOSP_Update_2obj_Hybrid(Graph* graph1, Graph* graph2, SOSPTree* tree1, SOSPTree* tree2, 
                               Edge* inserted_edges1, Edge* inserted_edges2, 
                               int num_inserted1, int num_inserted2, int source,
                               int rank, int size, Partition* partition) {
        ...
        if (rank == 0) {
            printf("Step 1: Updating SOSP trees with hybrid MPI+OpenMP parallelization...\n");
        }
        
        // Step 1: Update SOSP trees with hybrid parallelization
        double start_time_sosp1 = MPI_Wtime();
        SOSP_Update_Hybrid(graph1, tree1, inserted_edges1, num_inserted1, rank, size, partition);
        double end_time_sosp1 = MPI_Wtime();
        
        double start_time_sosp2 = MPI_Wtime();
        SOSP_Update_Hybrid(graph2, tree2, inserted_edges2, num_inserted2, rank, size, partition);
        ...
        if (rank == 0) {
            ...
            printf("Step 2: Creating combined graph with OpenMP parallelization...\n");
        }
        
        // Step 2: Create combined graph with OpenMP parallelism
        double start_time_combined = MPI_Wtime();
        Graph* combined = createGraph(n);
        
        // Add edges from both SOSP trees to combined graph in parallel
        #pragma omp parallel for
        for (int idx = partition->start_vertex; idx <= partition->end_vertex; idx++) {
        ...
        if (rank == 0) {
            ...
            printf("Step 3: Finding SOSP in combined graph with hybrid parallelization...\n");
        }
        
        // Step 3: Find SOSP in combined graph with hybrid parallelization
        double start_time_final = MPI_Wtime();
        SOSPTree* combined_tree = initSOSPTree(n, source);
        
        // Use hybrid Bellman-Ford to compute SOSP in combined graph
        computeInitialSOSP_Hybrid(combined, combined_tree, source, rank, size, partition);
        ...
        if (rank == 0) {
            ...
            // Find reachable vertices in parallel
            int reachable_count = 0;
            #pragma omp parallel
            {
                int local_count = 0;
    ...
    int main(int argc, char* argv[]) {
        ...
        if (rank == 0) {
            printf("Hybrid MPI+OpenMP configuration: %d MPI processes, %d OpenMP threads per process\n", 
                   size, num_threads);
        }
        ...
        // Set random seed based on rank and thread to avoid identical random numbers
        ...
        if (rank == 0) {
            printf("Hybrid MPI+OpenMP MOSP Update Implementation\n");
        }
        ...
        // Compute initial SOSP trees with hybrid parallelization
        if (rank == 0) {
            printf("Computing initial SOSP trees with hybrid parallelization...\n");
        }
        double start_time_initial = MPI_Wtime();
        computeInitialSOSP_Hybrid(graph1, tree1, source_vertex, rank, size, partition);
        computeInitialSOSP_Hybrid(graph2, tree2, source_vertex, rank, size, partition);
        ...
        // Run MOSP_Update with hybrid parallelization
        if (rank == 0) {
            printf("Running MOSP_Update with hybrid MPI+OpenMP parallelization...\n");
        }
        double start_time = MPI_Wtime();
        
        MOSP_Update_2obj_Hybrid(graph1, graph2, tree1, tree2, 
